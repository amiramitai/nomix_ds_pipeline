...
class FCN8VGGISH:
    ...
    def __init__(self, weights_path, dense_size=64):
        ...
        for k, _ in reader.get_variable_to_shape_map().items():
            self.data_dict[k] = reader.get_tensor(k)
        self.wd = 5e-4


    def build(self, images, train=False, num_classes=20, random_init_fc3=False, debug=False):
        """
        ...
        self.model = vggish.Vggish(images)
        self.model.load_weights(self.weights_path)

        self.pool5 = self.model.pool4
        self.pool4 = self.model.pool3
        self.pool3 = self.model.pool2
        self.pool2 = self.model.pool1

        self.fc1 = self._fc_layer(self.pool5, "fc1")
        ...
        if use_dilated:
            self.pool4 = tf.batch_to_space(self.pool4, crops=pad, block_size=2)
            self.pool4 = tf.batch_to_space(self.pool4, crops=pad, block_size=2)
            self.fc2 = tf.batch_to_space(self.fc2, crops=pad, block_size=2)
            self.fc2 = tf.batch_to_space(self.fc2, crops=pad, block_size=2)
            return

        if random_init_fc3:
            self.score_fr = self._score_layer(self.fc2, "fc3",
                                              num_classes)
        else:
            self.score_fr = self._fc_layer(self.fc2, "fc3",
    ...
    def _fc_layer(self, bottom, name, num_classes=2, relu=True, debug=False):
        with tf.variable_scope(name) as scope:
            shape = bottom.get_shape().as_list()

            if name == 'fc1':
                filt = self.get_fc_weight_reshape(name, [14, 14, 512, self.dense_size])
            elif name == 'fc3':
                filt = self.get_fc_weight_reshape(name, [1, 1, self.dense_size, 2],
                                                  num_classes=num_classes)
            else:
                filt = self.get_fc_weight_reshape(name, [1, 1, self.dense_size, self.dense_size])
    ...
    def get_deconv_filter(self, f_shape):
        ...
        f = ceil(width/2.0)
        c = (2 * f - 1 - f % 2) / (2.0 * f)
        bilinear = np.zeros([width, f_shape[1]])
