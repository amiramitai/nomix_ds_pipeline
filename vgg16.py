...
class Vgg16:
    ...
    def load_weights(self, weight_file, sess, weights_to_load_hack):
        print('[+] loading weights:')
        weights = np.load(weight_file)
        keys = sorted(weights.keys())
        # list(set(['_'.join(k.split('_')[:-1] for k in keys]))
...
if __name__ == '__main__':
    ...
    # img1 = imread('final.png', mode='RGB')
    ...
    with open(sys.argv[1], 'r') as f:
        for path in f:
            try:
                path = path.strip()
                i += 1
                # batch = [imread(path, mode='RGB') for path in images_paths[i:i+BATCH_SIZE]]
                dirname = os.path.dirname(path).replace('imgs_', 'embd_')
                filename = '.'.join(os.path.basename(path).split('.')[:-1]) + '.embed'
                out_path = os.path.join(dirname, filename)
                if os.path.exists(out_path):
                    print('{}. SKIPPING {}'.format(i, path))
                    continue
                print('{}. {}'.format(i, path))
                ...
                embed = sess.run(vgg.fc1, feed_dict={vgg.imgs: [img]})[0]
                pickle.dump(embed, open(out_path, 'wb'))
